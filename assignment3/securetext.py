...
class SecureTextServer:
    ...
    def get_user_pubkey(self, username):
        """Retrieve a user's public key (PEM format)."""
        pubkey = self.user_pubkeys.get(username)
        
        if pubkey:
            # Log when someone requests a public key (key exchange initiation)
            self.log_inspection_event(
                "ECDH_PUBKEY_REQUESTED", 
                username, 
                f"Public key requested for {username} (ECDH key exchange initiation)",
                {
                    "key_available": True,
            ...
            self.log_inspection_event(
                "ECDH_PUBKEY_REQUESTED", 
                username, 
                f"Public key requested for {username} but not available",
                {
                    "key_available": False
                }
            )
    ...
    def run(self):
        ...
        while True:
            if not self.logged_in:
                ...
                print("4. Start Inspection Mode (Admin only)")
                print("5. Stop Inspection Mode (Admin only)")
                print("6. Logout")
